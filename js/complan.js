...
//         {
//             "id": 1,
//             "characteristic": `Комплексний план с. Чермалик Розрахунковий показник – площа 1.222364 км2`,
//             "nameOfTheDocument": `Р.40 . Збірник цін на проектні роботи для будівництва. Районне планування. Планування і забудова населених пунктів. табл. 40 – 20. п.1. ДСТУ Б Д.1.1-7:2013 (зміна 1,2,3) K3=1.01-табл. Ж.1; K4=32.9- табл. Ж.З; п.2.`,
//             "costCalculation": `(53056 + 627,8 × 1,222364) × 1,01 × 32,9`,
//             "price": "1788497,76"
//         },
//         {
//             "id": 2,
...
function сountGenplan() {
    ...
    if (type == "село") {
        ...
        if (kilkistNaselenya > 10.0) {
            toastConstructor(toast, { type: 'success', text: `🤷‍♂️ Забагато народу ${kilkistNaselenya.toFixed(2)} тис. чол., мабуть це вже смт або місто, обери тип "місто" або "смт"` }); // Setup toast
            new bootstrap.Toast(toast).show(); // Show toast

        }

        toastConstructor(toast, { type: 'success', text: `🎉 Порахував генплан для ${nPunkt}` }); // Setup toast
        new bootstrap.Toast(toast).show(); // Show toast
        return
    }
    if (type == "місто" || type == "смт") {
        ...
        if (kilkistNaselenya > 2500.0) {
            toastConstructor(toast, { type: 'success', text: `🤷‍♂️ Забагато народу ${kilkistNaselenya.toFixed(2)} тис. чол., я вмію рахувати тільки до 2500 тис. чол.` }); // Setup toast
            new bootstrap.Toast(toast).show(); // Show toast
            return
        }
